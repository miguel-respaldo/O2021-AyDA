...
public class AlgoritmoKruskal {
    ...
    public AlgoritmoKruskal(int n) throws IOException {
    ...
    public boolean can_reach(int k, int l, int i) {
        ...
        for (int o = 1; o <= i; o++) {
            ...
            if ((k == t[o][1]) && !(temp[t[o][2]][l])) {
                if (can_reach(t[o][2], l, i))
                    return true;
            } else if ((k == t[o][2]) && !(temp[t[o][1]][l])) {
                if (can_reach(t[o][1], l, i))
                    return true;
            }
        }
        return false;
    ...
    public void getMinKL() {
        ...
        for (int i = 1; i <= n; i++)
            for (int j = 1; j <= n; j++) {
                if ((i < j)) {
                    if ((G[i][j] < G[k1][l1]) && G[i][j] != 0 && !in[j][i]) {
                        k1 = i;
                        l1 = j;
                    }
                }
    ...
    public static void main(String[] args) throws IOException {
        ...
        AlgoritmoKruskal kr = new AlgoritmoKruskal(n);
